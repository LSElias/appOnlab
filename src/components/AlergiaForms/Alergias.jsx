...
function TableAlergiasHead (props) {
  const {
    // onSelectAllClick,
    order,
    orderBy,
    ...
    <TableHead>
      <TableRow>
        <TableCell padding='checkbox'>
          <Tooltip title='Nuevo'>
            <IconButton component={Link} to='/alergias/create'>
              <AddIcon />
            </IconButton>
          </Tooltip>
        </TableCell>
...
function TableAlergiasToolbar (props) {
  const navigate = useNavigate()
  const { numSelected } = props
  const { idSelected } = props
  const update = () => {
    console.log(idSelected)
    return navigate(`/alergias/update/${idSelected}`)
  }
  return (
    <Toolbar
      sx={{
      ...
      {numSelected > 0
        ? (
          <Typography
            ...
            component='div'
          >
            {numSelected} alergias seleccionada/s
          </Typography>
          )
        : (
          <Typography
...
TableAlergiasToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
     ...
     {data && 
      ...
      <Box sx={{ width: '95%', marginX: 'auto', marginY: 2,pb:8}}>
          <Paper sx={{ width: '100%', mb: 1,  }}>
            <TableAlergiasToolbar numSelected={selected.length} idSelected={Number(selected[0]) || 0} />
            <TableContainer>
              <Table
                sx={{ minWidth: 750 }}
