...
export function EnfermedadesForm () {
  ...
  const enfermedadSchema=yup.object({
    nombre: yup.string().required('Ingrese el nombre de la enfermedad')
    .min(5, 'El nombre debe tener 5 caracteres'),
    descripcion: yup.string().required('Ingrese el nombre de la enfermedad')
  ...
  const { data, error, loaded } = useCallApi({ endpoint: `enfermedad/${id}` })
  ...
  useEffect(()=>{ 
    if(responseData!= null && responseData !="No se realizo el registro") {
      toast.success(esCrear ? "Enfermedad creada correctamente" : 'Actualización realizada correctamente.', {
        style: {
      ...
      setStart(false);
      if(responseData == "No se realizo el registro"){
