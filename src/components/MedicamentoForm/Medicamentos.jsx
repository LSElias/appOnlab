...
function TableMedicamentosHead (props) {
  const {
    // onSelectAllClick,
    order,
    orderBy,
    ...
    <TableHead>
      <TableRow>
        <TableCell padding='checkbox'>
          <Tooltip title='Nuevo'>
            <IconButton component={Link} to='/medicamentos/create'>
              <AddIcon />
            </IconButton>
          </Tooltip>
        </TableCell>
...
TableMedicamentosHead.propTypes = {
...
function TableMedicamentosToolbar (props) {
  const navigate = useNavigate()
  const { numSelected } = props
  const { idSelected } = props
  const update = () => {
    //console.log(idSelected)
    return navigate(`/medicamentos/update/${idSelected}`)
  }
  return (
    <Toolbar
      sx={{
      ...
      {numSelected > 0
        ? (
          <Typography
            ...
            component='div'
          >
            {numSelected} Medicamentos seleccionado/s
          </Typography>
          )
        : (
          <Typography
...
TableMedicamentosToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
     ...
     {data && 
      ...
      <Box sx={{ width: '95%', marginX: 'auto', marginY: 2,pb:8}}>
          <Paper sx={{ width: '100%', mb: 1,  }}>
            <TableMedicamentosToolbar numSelected={selected.length} idSelected={Number(selected[0]) || 0} />
            <TableContainer>
              <Table
                sx={{ minWidth: 750 }}
