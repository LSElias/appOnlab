...
export function MedicamentosForm () {
  ...
  const medicamentoSchema=yup.object({
    nombre: yup.string().required('Ingrese el nombre de la medicamento')
    .min(5, 'El nombre debe tener 5 caracteres'),
    descripcion: yup.string().required('Ingrese el nombre de la medicamento')
  ...
  useEffect(()=>{ 
    if(responseData!= null && responseData !="No se realizo el registro") {
      toast.success(esCrear ? "Medicamento creado correctamente" : 'Actualización realizada correctamente.', {
        style: {
