...
function TableConsultorioHead (props) {
  const {
    // onSelectAllClick,
    order,
    orderBy,
    ...
    <TableHead>
      <TableRow>
        <TableCell padding='checkbox'>
          <Tooltip title='Nuevo'>
            <IconButton component={Link} to='/Consultorio/create'>
              <AddIcon />
            </IconButton>
          </Tooltip>
        </TableCell>
...
TableConsultorioHead.propTypes = {
...
function TableConsultorioToolbar (props) {
      ...
      {numSelected > 0
        ? (
          <Typography
            ...
            component='div'
          >
            {numSelected} Consultorio seleccionado/s
          </Typography>
          )
        : (
          <Typography
...
TableConsultorioToolbar.propTypes = {
  numSelected: PropTypes.number.isRequired,
...
export function Consultorio () {
    ...
    const { data, error, loaded } = useCallApi({ endpoint: `Consultorio/GetUser/${userData.id}` })
    const [order, setOrder] = React.useState('asc')
    const [orderBy, setOrderBy] = React.useState('consultorio')
     ...
     {data && 
      ...
      <Box sx={{ width: '95%', marginX: 'auto', marginY: 2,pb:8}}>
          <Paper sx={{ width: '100%', mb: 1,  }}>
            <TableConsultorioToolbar numSelected={selected.length} idSelected={Number(selected[0]) || 0} />
            <TableContainer>
              <Table
                sx={{ minWidth: 750 }}
